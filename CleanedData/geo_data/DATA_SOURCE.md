# Geographic Data Source 地理数据来源

## File 文件
`world-countries.json`

## Source 来源
**Natural Earth**
- Website 网站: https://www.naturalearthdata.com/
- Data Product 数据产品: Natural Earth 1:110m Cultural Vectors - Admin 0 Countries
- Download Page 下载页面: https://www.naturalearthdata.com/downloads/110m-cultural-vectors/

## Description 描述
This GeoJSON file contains simplified country boundaries for world map visualization. The data is derived from Natural Earth's 1:110 million scale cultural vectors dataset.

该 GeoJSON 文件包含用于世界地图可视化的简化国家边界数据。数据来源于 Natural Earth 的 1:110 百万比例尺文化矢量数据集。

## Citation 引用方式

### For publications 学术出版物:
```
Made with Natural Earth. Free vector and raster map data @ naturalearthdata.com.
```

### BibTeX format:
```bibtex
@misc{naturalearthdata,
  author = {{Natural Earth}},
  title = {Natural Earth 1:110m Cultural Vectors},
  howpublished = {\url{https://www.naturalearthdata.com/}},
  note = {Free vector and raster map data}
}
```

## License 许可证
Natural Earth data is in the **public domain**. You may use it for any purpose without restriction.

Natural Earth 数据属于**公共领域**。您可以无限制地将其用于任何目的。

## Additional Information 额外信息
- Resolution 分辨率: 1:110 million (low resolution for overview maps)
- Format 格式: GeoJSON
- Coordinate System 坐标系统: WGS84 (EPSG:4326)
- Last Updated 最后更新: 2024
